...
@Component
public class MissionFeedWebSocketHandler extends TextWebSocketHandler {

    private static final Logger log = LoggerFactory.getLogger(MissionFeedWebSocketHandler.class);
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        ...
        log.info("Mission feed client connected: {} (total: {})", session.getId(), sessions.size());
    }

    @Override
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        sessions.removeIf(s -> s.getId().equals(session.getId()));
        log.info("Mission feed client disconnected: {} (total: {})", session.getId(), sessions.size());
    }
