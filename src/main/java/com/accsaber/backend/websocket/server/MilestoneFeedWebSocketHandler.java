...
@Component
public class MilestoneFeedWebSocketHandler extends TextWebSocketHandler {

    private static final Logger log = LoggerFactory.getLogger(MilestoneFeedWebSocketHandler.class);
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        ...
        log.info("Milestone feed client connected: {} (total: {})", session.getId(), sessions.size());
    }

    @Override
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        sessions.removeIf(s -> s.getId().equals(session.getId()));
        log.info("Milestone feed client disconnected: {} (total: {})", session.getId(), sessions.size());
    }
