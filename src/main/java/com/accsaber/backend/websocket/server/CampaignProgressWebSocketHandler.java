...
@Component
public class CampaignProgressWebSocketHandler extends TextWebSocketHandler {

    private static final Logger log = LoggerFactory.getLogger(CampaignProgressWebSocketHandler.class);
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        ...
        log.info("Campaign progress feed client connected: {} (total: {})", session.getId(), sessions.size());
    }

    @Override
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        sessions.removeIf(s -> s.getId().equals(session.getId()));
        log.info("Campaign progress feed client disconnected: {} (total: {})", session.getId(), sessions.size());
    }
