...
public class NewsController {
    ...
    @Operation(summary = "List published news", description = "Pinned posts first, newest published first")
    @GetMapping
    public ResponseEntity<Page<PublicNewsResponse>> list(@PageableDefault(size = 20) Pageable pageable) {
        return ResponseEntity.ok(newsService.findPublic(pageable));
    }
