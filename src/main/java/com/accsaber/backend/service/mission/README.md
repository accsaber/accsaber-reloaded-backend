...
Each branch also carries a `minAp` floor (`snipeMinAp` = `max(target * snipeFloorFraction(band), userCurrentAp)`). The target is what calibration decided you should beat, so a candidate far *below* it is not a snipe - it's busywork wearing the band's name. Anything under the floor is rejected and the map resampled. This is the lower half of a band around the target: `slack` bounds how far above it a candidate may sit, `snipeFloorFraction` how far below. Without it, `pickSnipeCandidate` returns "closest to target" even when the closest thing on the map is 150 AP short.
...
- **`snipeSlack(band)` (1.00/1.01/1.03/1.04)** - the upper half of the band around the target, and it now applies to **both** snipe branches. It must stay strictly above `snipeFloorFraction` for the same band or that band becomes unassignable: the viable window is `[target * floorFraction, target * slack]`, and if the floor crosses the slack there is no AP a candidate could have. This is why the has-score branch can't go back to `apCap = target` - with extreme's floor at 1.01 that window is empty and every extreme snipe on a map you've played would fail forever.

  Both knobs decide *quality*; the window width they leave decides *how often snipes get assigned at all*. Measured on dev with 0.95/0.97/0.99/1.01, per-map viability is 8.5/11.8/13.1/13.2% and an 8-attempt build succeeds ~51/64/68/68% of the time (vs ~53/65/70/71% at 0.93/0.95/0.96/0.97 - the floor is cheap). The ~30-50% failure rate is driven by the **slack** cap, not the floor, and a failed snipe falls through to another template rather than costing a slot. If snipes feel too rare, widen `snipeSlack` or raise `MAP_SAMPLE_ATTEMPTS`; don't drop the floor.
